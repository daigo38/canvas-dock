...
function Checkbox({
  ...
  return (
    <CheckboxPrimitive.Root className={cn(
              ...
              className
            )} checked={isChecked} onCheckedChange={(value) => {
              setIsChecked(!!value)
              onCheckedChange?.(value)
            }} {...props} render={<motion.button className="flex items-center justify-center p-0" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} />}><svg
                viewBox="0 0 14 14"
                className="h-4 w-4"
                fill="none"
                strokeWidth={2}
